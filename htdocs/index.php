...
// the response.
$response = null;
$routeInfo = $dispatcher->dispatch($method, $path); //Router checks matching, and dispatches
switch ($routeInfo[0]) {
    case FastRoute\Dispatcher::NOT_FOUND: //The route was not found
        $response = (new Error())->do404(); //Create a response object that gives a 404 (from Error-class in Controller)
        break;
    ...
    case FastRoute\Dispatcher::FOUND: //If there is a match we call a handler that returns a response
        $handler = $routeInfo[1];
        $vars = $routeInfo[2];
...
} else if (is_string($response)) { //Returning only a string for e.g. debug
    echo $response;
} else {
    (new Emitter())->emit($response); //New Emitter object and using a method immediately in one line
}
